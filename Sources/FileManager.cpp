...
FileHandle FileManager::open(const string &filePath, unsigned long fileAccess, bool exists, bool isTmp)
{
    ...
    if (_tmpFileHandles.size() >= MAX_FD)
    {
...
#if defined(_WIN64)
    fileHandle = CreateFileA(filePath.c_str(), fileAccess, FILE_SHARE_READ, NULL, exists ? OPEN_EXISTING : CREATE_ALWAYS, 0, NULL);
...
FileHandle FileManager::create(const string &filePath)
{
    FileHandle fileHandle = open(filePath, RDWR, false, false);
    _fileHandlesCache.insert(fileHandle);
    return (fileHandle);
}
...
void FileManager::lockOutFileMutex()
{
#if defined(WINDOWS)
    _outFileMutex.lock();
#elif defined(LINUX)
...
void FileManager::unlockOutFileMutex()
{
#if defined(WINDOWS)
    _outFileMutex.unlock();
#elif defined(LINUX)
#endif
}
